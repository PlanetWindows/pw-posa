(() => {
  ...
  const B='pw-assistance-private', D='pw-ddt-private';
  ...
  async function blob(bucket,path){const r=await sb.storage.from(bucket).download(path);if(r.error)throw r.error;return r.data}
  ...
  async function exportAssistance(id,btn){
    ...
    try{
      ...
      const [pr,dr]=await Promise.all([
        sb.from('assistance_photos').select('*').eq('assistance_id',id).order('created_at'),
        sb.from('ddt_documents').select('*').eq('assistance_id',id).maybeSingle()
      ]);
      if(pr.error)throw pr.error;if(dr.error)throw dr.error;const photos=pr.data||[],ddt=dr.data||null;
      const zip=new JSZip(),pf=zip.folder('Fotografie'),rf=zip.folder('Rapportino_Assistenza'),df=zip.folder('DDT'),missing=[];
      let ddtFiles=0;
      zip.file('Dati_Assistenza.txt',[
        'PW Posa - Fascicolo assistenza','',`Protocollo / ordine: ${a.protocol_order||'—'}`,`Cliente: ${a.client_name||'—'}`,`Telefono: ${a.client_phone||'—'}`,`Email: ${a.client_email||'—'}`,`Indirizzo: ${a.address||'—'}${a.city?`, ${a.city}`:''}${a.postal_code?` ${a.postal_code}`:''}`,`Data assistenza: ${fd(a.scheduled_date)}${a.scheduled_end_date&&a.scheduled_end_date!==a.scheduled_date?' → '+fd(a.scheduled_end_date):''}`,`Problematica: ${a.final_issue_description||a.issue_description||'—'}`,`Intervento eseguito: ${a.work_performed||a.final_work_performed||a.intervention_description||'—'}`,`Esito / risoluzione: ${a.resolution_status||a.final_resolution_status||a.resolved||'—'}`,`Note finali: ${a.final_notes||a.installer_notes||'—'}`,`DDT incluso: ${ddt?'Sì':'No'}`,`Stato email DDT: ${ddt?.email_status||'—'}`
      ].join('\r\n'));
      if(a.final_report_path){try{rf.file(a.final_report_name||'Rapportino_Assistenza.pdf',await blob(B,a.final_report_path))}catch(e){missing.push('Rapportino: '+e.message)}}
      let i=0;for(const ph of photos){if(!ph.storage_path)continue;try{const b=await blob(B,ph.storage_path);i++;pf.file(`Foto_${String(i).padStart(2,'0')}${ext(ph.storage_path,b)}`,b)}catch(e){missing.push(`Foto ${ph.file_name||ph.id}: ${e.message}`)}}
      if(ddt){
        if(ddt.original_path){try{df.file(`DDT_Originale_${safe(ddt.original_name||'documento.pdf')}`,await blob(D,ddt.original_path));ddtFiles++;}catch(e){missing.push('DDT originale: '+e.message)}}
        if(ddt.signed_path){try{df.file(`DDT_Firmato_${safe(ddt.signed_name||'DDT_firmato.pdf')}`,await blob(D,ddt.signed_path));ddtFiles++;}catch(e){missing.push('DDT firmato: '+e.message)}}
        df.file('Stato_DDT.txt',[`Email cliente: ${a.client_email||'—'}`,`Stato invio: ${ddt.email_status||'pending'}`,`Firmato il: ${ddt.signed_at?new Date(ddt.signed_at).toLocaleString('it-IT'):'—'}`].join('\r\n'));
      }
