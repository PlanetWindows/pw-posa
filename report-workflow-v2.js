(() => {
  ...
  async function enhance(){
    ...
    if(completed){
      ...
      completed.value=pose?.office_notes||completed.value||'';completed.readOnly=true;completed.required=false;completed.placeholder='Descrizione inserita dall’Ufficio';completed.style.background='#f5f3ef';
    }
    const grid=form.querySelector('.report-grid');if(!grid)return;
    const remaining=$('reportRemaining')?.closest('label'),reason=$('reportReason')?.closest('label'),issues=$('reportIssues')?.closest('label'),materials=$('reportMaterials')?.closest('label'),notes=$('reportNotes')?.closest('label');
    ...
    const choice=document.createElement('div');choice.className='span2 report-finished-choice';choice.innerHTML='<strong>Il lavoro è stato finito? *</strong><div style="display:flex;gap:22px;margin-top:9px"><label style="display:flex;align-items:center;gap:7px;font-weight:600"><input type="radio" name="pwFinished" value="yes" required style="width:auto"> Sì</label><label style="display:flex;align-items:center;gap:7px;font-weight:600"><input type="radio" name="pwFinished" value="no" required style="width:auto"> No</label></div>';
    ...
    const setMode=v=>{conditional.forEach(x=>x.style.display=v==='no'?'':'none');if(v==='yes'){if($('reportRemaining'))$('reportRemaining').value='';if($('reportReason'))$('reportReason').value='';if($('reportIssues'))$('reportIssues').value='';}};
