(()=>{
  ...
  function ensureConfirmDialog(){
    let dlg=$('deleteAssistanceDialog');
    if(dlg)return dlg;
    dlg=document.createElement('dialog');
    dlg.id='deleteAssistanceDialog';
    dlg.className='delete-pose-dialog';
    dlg.innerHTML=`<div class="delete-pose-inner">
      <div class="eyebrow">CONFERMA ELIMINAZIONE</div>
      <h3 id="deleteAssistanceTitle">Eliminare l'assistenza?</h3>
      <p id="deleteAssistanceText">Questa operazione eliminerà definitivamente l'assistenza e i dati collegati.</p>
      <div class="delete-pose-actions">
        <button type="button" class="btn ghost" id="deleteAssistanceCancel">Annulla</button>
        <button type="button" class="btn delete-pose-danger" id="deleteAssistanceConfirm">Elimina assistenza</button>
      </div>
    </div>`;
    document.body.appendChild(dlg);
    $('deleteAssistanceCancel').addEventListener('click',()=>dlg.close());
    dlg.addEventListener('cancel',e=>{e.preventDefault();dlg.close()});
    $('deleteAssistanceConfirm').addEventListener('click',()=>removeAssistance());
    return dlg;
  }

  async function removeAssistance(){
    if(!id||busy)return;
    if(!canDelete())return toast('Solo l’Ufficio che gestisce il calendario può eliminare assistenze');
    busy=true;
    ...
    try{
      ...
      await sb.from('ddt_documents').delete().eq('assistance_id',id);
      await sb.from('assistance_push_events').delete().eq('assistance_id',id);
      await sb.from('assistance_photos').delete().eq('assistance_id',id);
      await sb.from('assistance_dates').delete().eq('assistance_id',id);
      const r=await sb.from('assistances').delete().eq('id',id).select('id');
      if(r.error)throw r.error;
      if(!r.data?.length)throw new Error('L’assistenza non è stata eliminata. Verifica la policy DELETE di Supabase.');
      $('deleteAssistanceDialog')?.close();
  ...
  async function ensureDeleteButton(){
    const detail=$('assistanceDetailDialog');
    if(!detail?.open||!id)return;
    if(!canDelete()){detail.querySelector('#deleteAssistanceBtn')?.remove();return;}
    const actions=detail.querySelector('.detail-head-actions');if(!actions)return;
    let btn=$('deleteAssistanceBtn');
    if(!btn){
      btn=document.createElement('button');
      btn.type='button';
      btn.id='deleteAssistanceBtn';
      ...
      btn.addEventListener('click',()=>{
        ...
        $('deleteAssistanceTitle').textContent=`Eliminare l'assistenza ${protocol||''}?`;
        $('deleteAssistanceText').textContent='Confermando, l’assistenza verrà eliminata definitivamente insieme ai dati collegati. Questa operazione non può essere annullata.';
  ...
  document.addEventListener('click',e=>{
    ...
    if(a?.dataset.assistance){
      id=a.dataset.assistance;
      protocol=(a.textContent||'').match(/ASSISTENZA\s+([^\n]+)/i)?.[1]?.trim()||'';
      setTimeout(ensureDeleteButton,80);
    }
  },true);

  const detail=$('assistanceDetailDialog');
  if(detail)new MutationObserver(()=>setTimeout(ensureDeleteButton,0)).observe(detail,{attributes:true,attributeFilter:['open']});
