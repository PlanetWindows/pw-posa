(()=>{
  ...
  let role=null,id=null,protocol='',busy=false,detailObserver=null;
  ...
  async function removeAssistance(){
    ...
    try{
      const [ph,a,ddt]=await Promise.all([
        sb.from('assistance_photos').select('storage_path').eq('assistance_id',id),
        sb.from('assistances').select('final_report_path,signed_document_path,summary_document_path').eq('id',id).single(),
        sb.from('ddt_documents').select('original_path,signed_path').eq('assistance_id',id)
      ]);
      ...
      await Promise.all([
        sb.from('ddt_documents').delete().eq('assistance_id',id),
        sb.from('assistance_push_events').delete().eq('assistance_id',id),
        sb.from('assistance_photos').delete().eq('assistance_id',id),
        sb.from('assistance_dates').delete().eq('assistance_id',id)
      ]);
  ...
  async function resolveActiveAssistance(){
    ...
    const title=String($('assDetailTitle')?.textContent||'').trim();if(title.includes('·'))protocol=title.split('·')[0].trim();
    if(id)return true;if(!protocol)return false;
  ...
  async function ensureDeleteButton(){
    ...
    const actions=detail.querySelector('.detail-head-actions');if(!actions||$('deleteAssistanceBtn'))return;
    const btn=document.createElement('button');btn.type='button';btn.id='deleteAssistanceBtn';btn.className='btn ghost delete-pose-btn';btn.innerHTML='🗑 Elimina';
  ...
  function watchDetailDialog(){
    const detail=$('assistanceDetailDialog');if(!detail||detailObserver)return false;
    detailObserver=new MutationObserver(()=>{if(detail.open)setTimeout(ensureDeleteButton,0)});
    detailObserver.observe(detail,{attributes:true,attributeFilter:['open']});return true;
  }

  document.addEventListener('click',e=>{
    const a=e.target.closest('[data-assistance]');
    if(!a?.dataset.assistance)return;
    id=a.dataset.assistance;protocol='';setTimeout(ensureDeleteButton,60);
  },true);

  window.addEventListener('load',async()=>{
    await init();
    if(!watchDetailDialog()){
      let tries=0;const t=setInterval(()=>{tries++;if(watchDetailDialog()||tries>=20)clearInterval(t)},100);
    }
  });
})();